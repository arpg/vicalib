...
static ImuPoseT<T> IntegratePoseJet(const ImuPoseT<T>& pose,
  ...
  ImuPoseT<T> y = pose;
  y.t_wp.translation() += k.template head<3>() * static_cast<T>(dt);
  memcpy(
      y.t_wp.so3().data(),
  ...
  y.v_w += k.template tail<3>() * static_cast<T>(dt);
  return y;
}
